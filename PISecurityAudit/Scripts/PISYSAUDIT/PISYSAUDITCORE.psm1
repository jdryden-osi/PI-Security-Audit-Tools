...
function New-PISysAudit_BowTieTemplate
{
<#
.SYNOPSIS
Generates a Bow Tie diagram template. 
.DESCRIPTION
Accepts the path to a JSON file containing a BowTie definition,
returns a Bow Tie diagram template in SVG format. 
#>
[CmdletBinding(DefaultParameterSetName="Default", SupportsShouldProcess=$false)]     
param(
		[parameter(Mandatory=$true, Position=0, ParameterSetName = "Default")]
...
{		
	...
	# Load JSON definition file and convert to JSON
	$bowtieDefinition = Get-Content $DefinitionPath
	try
	{
	...
	{
		$msg1 = "Failed to load BowTie definition file at $DefinitionPath"
		Write-PISysAudit_LogMessage $msg "Error" $fn -eo $_
		return $null
	}

	# Extract threats (left side) and impacts (right side)
	$threats = $bowtieObject.Definition.Threats
	$impacts = $bowtieObject.Definition.Impacts

	# Defense/Reduction box height
	$drHeight = 60

	# Initialize SVG doc with headers and CSS styling
	$svgDoc = @"
...
"@
	...
	# Function to split text for line wrapping. 
	# Accepts the unsplit text, the width (pixels), upper and lower
	#     Y values (will be used to center text vertically), and a
	#     scale. Scale is 1 for 12pt font, 1.5 for 18pt, etc.
	# Returns array of lines, each with property 'text' and 'y'.
	function SplitText($text, $width, $upperY, $lowerY, $scale=1)
	{
		# Average of 6.34px per character at 12pt font
	...
	# Initial X and Y values for padding
	$initialX = 20
	$initialY = 70

	# Find Centerline, to hold middle circle
	# Circle is centered vertically on the side with fewer branches. May not be less than 260.
	if($threats.Count -ge $impacts.Count ) { $fewer = $impacts.Count } else { $fewer = $threats.Count }
	$circleY = $initialY + 120*($fewer/2) - 10
	if($circleY -lt 260) { $circleY = 260 }
	...
	foreach($threat in $threats)
	{
		# Move x forward if not longest branch
		...
		{
			$x += 120
			$svgDoc += "`t<rect x=`"$x`" y=`"$y`" width=`"100`" height=`"$drHeight`""
			if($defense.AuditID) { $svgDoc += " class=`"%$($defense.AuditID)%`"" }
			$svgDoc += "/>`r`n"
			$splitText = SplitText $defense.Name 100 $y ($y+$drHeight)
			foreach($line in $splitText)
			{
				$svgDoc += "`t<text x=`"$($x+50)`" y=`"$($line.y)`">$($line.text)</text>`r`n"
			}
			if($defense.AuditID)
			{
				$svgDoc += "`t<text x=`"$($x+50)`" y=`"$($y+$drHeight+12)`">$($defense.AuditID)</text>`r`n"
			}
		}
	...
	foreach($line in (SplitText $bowtieObject.TopEvent 150 ($circleY-75) ($circleY+75) 1.5))
	{
	...
	foreach($line in (SplitText $bowtieObject.Hazard 170 ($circleY-185) ($circleY-110) 1.5))
	{
		$svgDoc += "`t<text x=`"$x`" y=`"$($line.y)`" class=`"title2`">$($line.text)</text>`r`n"
	}
	...
	foreach($impact in $impacts)
	{
		...
		# Lines first
		$svgDoc += "`t<line x1=`"$x`" y1=`"$y`" x2=`"$($x+100+120*$impact.Reductions.Count)`" y2=`"$y`"/>`r`n"
		$svgDoc += "`t<line x1=`"$linesMeetX`" y1=`"$circleY`" x2=`"$rightHalfX`" y2=`"$y`"/>`r`n"

		# Draw reductions
		$y -= ($drHeight/2)
		foreach($reduction in $impact.Reductions)
		{
			$svgDoc += "`t<rect x=`"$x`" y=`"$y`" width=`"100`" height=`"$drHeight`""
			if($reduction.AuditID) { $svgDoc += " class=`"%$($reduction.AuditID)%`"" }
			$svgDoc += "/>`r`n"
			$splitText = SplitText $reduction.Name 100 $y ($y+$drHeight)
			foreach($line in $splitText)
			{
				$svgDoc += "`t<text x=`"$($x+50)`" y=`"$($line.y)`">$($line.text)</text>`r`n"
			}
			if($reduction.AuditID)
			{
				$svgDoc += "`t<text x=`"$($x+50)`" y=`"$($y+$drHeight+12)`">$($reduction.AuditID)</text>`r`n"
			}
			$x += 120
		}
	...
	$maxImpacts = 0
	foreach($impact in $impacts) {
		if($impact.Reductions.Count -gt $maxImpacts) { $maxImpacts = $impact.Reductions.Count }
	}
...
{		
	...
	try
	{
		...
		$daDefinitionPath = PathConcat -ParentPath $scriptsPath -ChildPath "Utilities\BowTieDefinition_PIDataArchive.json"
		$daTemplate = New-PISysAudit_BowTieTemplate -def $daDefinitionPath

		# PI AF Server: load template
		$afDefinitionPath = PathConcat -ParentPath $scriptsPath -ChildPath "Utilities\BowTieDefinition_PIAFServer.json"
		$afTemplate = New-PISysAudit_BowTieTemplate -def $afDefinitionPath
